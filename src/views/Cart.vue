      ...
      <div class="col-lg-12 text-left">
        ...
        <div class="cart" v-if="!loading">
          <div v-if="products.length > 0">
            <table class="responsive-table">
              ...
              <tbody>
                <tr v-for="product in products" :key="product.id">
                  ...
                  <td>
                    <router-link
                      :to="{ name: 'product', params: { url: product.isbn }}"
                    >{{ product.title }}</router-link>
                  </td>
                  <td>{{ product.price | priceFormatterFilter }}</td>
                  <td>
                    <button
                      class="btn btn-small waves-effect waves-light"
                      @click="decAmount(product)"
                    >-</button>
                    {{ calcProductAmount(product) }}
                    <button
                      class="btn btn-small waves-effect waves-light"
                      @click="incAmount(product)"
                    >+</button>
                  </td>
                  <td>{{ calcProductTotalPrice(product) | priceFormatterFilter }}</td>
                  <td>
            ...
            <div
              class="col s12 order-total-block"
            >Order Total: {{ cartTotalPrice | priceFormatterFilter }}</div>
              ...
              <div class="row">
                ...
                <div class="input-field col s6">
                  ...
                  <span
                    class="helper-text"
                    data-error="wrong"
                    v-if="!$v.form.name.required && $v.$dirty"
                  >Name must be required!</span>
                  <span
                    class="helper-text"
                    data-error="wrong"
                    v-if="!$v.form.name.minLength && $v.$dirty"
                  >Name must have at least {{ $v.form.name.$params.minLength.min }} letters!</span>
                </div>
                ...
                <div class="input-field col s6">
                  ...
                  <span
                    class="helper-text"
                    data-error="wrong"
                    v-if="!$v.form.email.required && $v.$dirty"
                  >Email must be required!</span>
                  <span
                    class="helper-text"
                    data-error="wrong"
                    v-if="!$v.form.email.email && $v.$dirty"
                  >Incorrect format of email!</span>
                </div>
                ...
                <div class="input-field col s12 right-align">
                  <button
                    ...
                    v-if="!isCheckout"
                  >Checkout</button>
                  <button type="button" class="btn" v-if="isCheckout" disabled>
                    Loading...
                    <span
